...
// voice and percussion mode
  if (mode == 0) { // Note On / Off pitch and velocity channels 1 - 8 controls gate, pitch CV and velocity CV
    ...
    MIDI.setHandleControlChange(OnCC_mode_1);
...
void OnNoteOn(byte channel, byte pitch, byte velocity) {
  if (channel < 3) {
   ...
   switch (channel) {
    ...
    writeDAC(45, dac_channel, constrain(map((pitch - offset_pitch) * 100.0 + pitchbend_value[channel - 1], 0.0, voltage_range, 0.0, 4095.0), 0.0, 4095.0)); //Notte CV
    
    
    //writeDAC(cs_pin - channel + 1, 0, map(velocity, 0, 127, 0, 4095)); //Velocity CV
...
void OnCC_mode_1(byte channel, byte cc_number, byte cc_value) {
  if(channel == 7) { //Si es CC del canal 7
    if(cc_number < 16) {

      writeDAC(45, 0, cc_value << 5); //Primer canal del DAC 0

      Serial.println("CC CHANNEL: ");
      Serial.println(channel);
      Serial.println("CC NUMBER: ");
...
void myClock() {
   ...
   if (play_flag == 1) {
    ...
    digitalWriteFast(8, 1 - bitRead(clock_tick / 3, 0)); // sixteenths
    digitalWriteFast(7, 1 - bitRead(clock_tick / 6, 0)); // eights
    digitalWriteFast(10, 1 - bitRead(clock_tick / 6, 0)); // eights
    digitalWriteFast(LED, 1 - bitRead(clock_tick / 12, 0)); // quarter note LED flash
